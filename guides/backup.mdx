...
- **`override = false`** — the method returns `false` immediately without writing anything. The existing destination file is left completely untouched. Use this when you want idempotent daily snapshots (one file per day at most) or when losing a previous backup would be unacceptable.
- **`override = true`** — the destination file is overwritten with the current configuration contents. The destination path must point to a regular file (not a directory) and the process must have write permission on it.
